...
const startInsertExchangeRateData = async () => {
    try {
        ...
        const standardUTCTime = getKoreaTime().setHours(8,50,0,0);
        ...
        if (enterTime > standardKSTTime
            ...
            const setResult = await repository.setExchangeRateIFData(finishedInsertMapArray)
                .catch(ex => {
                    // TEAMS
            ...
            /**
             * @dev
             * 1. 1회차 데이터가 아닐 시 UFSP에 등록된 환율 정보 9999년도로 이전.
             * 2. 9999년도에 이미 등록된 데이터가 존재 시 1년식 내려서 등록 시도.
             */
            if (!setResult) {
                teams.addProcessResult("[원복] 1회차 데이터 X");
                const sendDataList = JSON.parse(JSON.stringify(ufsp.resultData.exchange_rate_insert_info));
                if (sendDataList) {
                    const revertScript = await repository.getUFSPExchangeRateRevertScript()
                        .catch(ex => {
                            teams.sendMessage(process10, ex, false);
                            throw "error";
                        });
                    for (const data of sendDataList) {
                        for (let i=9999; i>=2000; i--) {
                            ufsp.resultData = {};

                            const originRow = util.setUFSPOriginRow(data);
                            const updateRow = util.setUFSPUpdateRow(insertRateData, data, i);

                            await ufsp.addJsonResult('exchange_rate_data', '', '', originRow, 'addBulk');
                            await ufsp.addJsonResult('data_insert_time', '', '', convertCurrentTimeToUFSPFormat(),'addBulk');
                            await ufsp.addJsonResult('exchange_rate_update_data', '', '', updateRow, 'addBulk');

                            await ufsp.startScript(revertScript)
                                .catch(async ex => {
                                    teams.sendMessage(process10, ex, false);
                                    throw "error";
                                });

                            const result = await ufsp.resultData.exchange_rate_revert_result[0];
